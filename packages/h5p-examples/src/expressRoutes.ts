...
export default function (
    ...
    router.get(
        `${h5pEditor.config.playUrl}/:contentId`, authenticateToken,
    ...
    router.get(
        '/edit/:contentId', authenticateToken,
    ...
    router.post('/edit/:contentId', authenticateToken, async (req: IRequestWithUser, res) => {
    ...
    router.post('/new', authenticateToken, async (req: IRequestWithUser, res) => {
        if (
            !req.body.params ||
            !req.body.params.params ||
            !req.body.params.metadata ||
    ...
    router.get('/delete/:contentId', authenticateToken, async (req: IRequestWithUser, res) => {
        try {
    ...
    router.get('/sample', authenticateToken,  async (req: IRequestWithUser, res) => {
        res.send('Hello World!')
    })

    router.get(
        '/:contentId/edit', authenticateToken,
    ...
    router.get('/', authenticateToken, async (req: IRequestWithUser, res) => {
        let contentObjects;
        try {
    ...
    router.post('/', authenticateToken, async (req: IRequestWithUser, res) => {
        if (
            !req.body.params ||
            !req.body.params.params ||
            !req.body.params.metadata ||
    ...
    router.delete('/:contentId', authenticateToken, async (req: IRequestWithUser, res) => {
        try {
            await h5pEditor.deleteContent(req.params.contentId, req.user);
        } catch (error) {
            console.error(error);
    ...
    router.get(`/:contentId/play`, authenticateToken, async (req: IRequestWithUser, res) => {
        try {
